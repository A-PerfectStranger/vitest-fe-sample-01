...
describe('Tarea 2', () => {
  it('debe eliminar el elemento de la lista al hacer clic en el botón eliminar', () => {
    // Arrange
    const lista = crearLista();
    const li = crearTareaElemento('Tarea con botón eliminar');
  ...
  it('debe alternar la clase "completada" al disparar el evento change en el checkbox', () => {
    ...
    // Act: marcar el checkbox como true y disparar el evento change
    checkbox.checked = true;
    checkbox.dispatchEvent(new Event('change'));

    // Assert: la clase "completada" debe haberse agregado
    expect(li.classList.contains('completada')).toBe(true);

    // Act: marcar el checkbox como false y disparar el evento change
    checkbox.checked = false;
    checkbox.dispatchEvent(new Event('change'));

    // Assert: la clase debe haberse quitado
    expect(li.classList.contains('completada')).toBe(false);
  });

  it('debe aceptar texto de exactamente 200 caracteres como válido', () => {
    // Arrange
    const lista = crearLista();
  ...
  it('debe dejar la lista vacía cuando todas las tareas están completadas y se llama limpiarCompletadas', () => {
    // Arrange
    const lista = crearLista();
    agregarTarea('Primera tarea', lista);
    agregarTarea('Segunda tarea', lista);
    agregarTarea('Tercera tarea', lista);
    ...
    expect(lista.querySelectorAll('.completada').length).toBe(3);
